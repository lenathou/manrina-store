    ...
    toCommandToShow = () => {
        ...
        return {
            ...
            order: {
                ...
                'Paid at': (successPayload?.created || 0) * 1000,
                'Payment method': successPayload?.payment_method_types?.[0],
                'Order status': successPayload?.status,
                'Subtotal': parseStripeAmount(successPayload?.amount_subtotal),
                'Shipping': this.basket.deliveryCost || 0,
                'Taxes': parseStripeAmount(successPayload?.total_details?.amount_tax),
                'Discount': parseStripeAmount(successPayload?.total_details?.amount_discount),
                'Total': parseStripeAmount(successPayload?.amount_total),
